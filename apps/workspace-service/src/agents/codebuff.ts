      ...
      .run({
        ...
        overrideTools: {
          ask_user: async (input) => {
            ...
            type AskUserOutput = [{ type: "json"; value: { answers?: Array<{ questionIndex: number; selectedOption?: string; selectedOptions?: string[]; otherText?: string }>; skipped?: boolean } }]
            ...
            const result = await new Promise<AskUserOutput>((resolve, reject) => {
              ...
              console.log("[Codebuff] Promise created - SDK is now BLOCKED waiting for user response")
        ...
        handleEvent: (e) => {
          ...
          if (e.type === "tool_call") {
            ...
            if (callToolName === "ask_user") {
              console.log("[Codebuff] handleEvent received ask_user tool_call (should be handled by overrideTools)")
              return
            }
