...
async function handleWorkspaceMessage(ws: ServerWebSocket<WSData>, msg: unknown): Promise<void> {
  ...
  switch (channelMsg.channel) {
    ...
    case "files":
      ...
      log.info("received files channel message", {
        type: channelMsg.type,
        hasFileOps: !!ws.data.fileOperations,
        isFileOpRequest: isFileOperationRequest(msg),
      });
      if (isFileOperationRequest(msg) && ws.data.fileOperations) {
        ...
        log.info("file operation complete, sending response", {
          type: response.type,
          success: "success" in response ? response.success : undefined,
        });
        ws.send(JSON.stringify(response));
      } else {
        log.warn("files channel message not handled", {
          type: channelMsg.type,
          hasFileOps: !!ws.data.fileOperations,
          isFileOpRequest: isFileOperationRequest(msg),
        });
    ...
    default:
      ...
      if (
        channelMsg.type === "prompt" ||
        channelMsg.type === "abort" ||
        channelMsg.type === "settings"
      ) {
