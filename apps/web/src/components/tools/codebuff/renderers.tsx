...
function ReadFilesRenderer({ input, result, error }: ToolRendererProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {result && !error && <CodeDisplay code={result} path={params.paths[0]} />}
    </div>
  );
}
...
function WriteFileRenderer({ input, result, error }: ToolRendererProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {params.instructions && <p className="text-xs text-zinc-500">{params.instructions}</p>}
      ...
      {result && !error && <div className="text-xs text-green-400">File written successfully</div>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        {params.cwd && <span className="text-xs text-zinc-500">in {params.cwd}</span>}
        {params.flags && <span className="text-xs text-zinc-600">{params.flags}</span>}
      </div>
      {error ? (
        <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-3 text-sm text-red-400">
          {error}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-zinc-300">Spawning {params.agents.length} agent(s)</span>
      </div>
      <div className="space-y-2">
        {params.agents.map((agent, i) => (
          <div key={i} className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-3 space-y-2">
            ...
            {agent.prompt && <p className="text-xs text-zinc-400 line-clamp-2">{agent.prompt}</p>}
          </div>
        ))}
      </div>
      {error && (
...
function SpawnAgentInlineRenderer({ input, result, error }: ToolRendererProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {params.prompt && <p className="text-xs text-zinc-400 line-clamp-2">{params.prompt}</p>}
      {error && (
        <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-3 text-sm text-red-400">
          {error}
        </div>
      )}
      {result && !error && <div className="text-xs text-green-400">Agent spawned successfully</div>}
    </div>
  );
}
...
function SetMessagesRenderer({ result, error }: ToolRendererProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {message && !error && <div className="text-xs text-zinc-400">{message}</div>}
    </div>
  );
}
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        {params.status && (
          <Badge
            variant="secondary"
            className={`text-xs ${subgoalStatusColors[params.status] || ""}`}
          >
      ...
      {params.plan && <p className="text-xs text-zinc-500">{params.plan}</p>}
      {error && (
        <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-3 text-sm text-red-400">
          {error}
        </div>
      )}
      {result && !error && <div className="text-xs text-green-400">Subgoal added</div>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <code className="text-xs bg-zinc-800 px-1.5 py-0.5 rounded text-zinc-400">{params.id}</code>
        {params.status && (
          <Badge
            variant="secondary"
            className={`text-xs ${subgoalStatusColors[params.status] || ""}`}
      ...
      {params.plan && <p className="text-xs text-zinc-400">{params.plan}</p>}
      {params.log && <p className="text-xs text-zinc-500 italic">{params.log}</p>}
      {error && (
        <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-3 text-sm text-red-400">
          {error}
        </div>
