...
import type { AgentType, ServerMessage, AgentSettings, PromptContext } from "@/types/agent";
...
export function useWorkspaceConnection({
  ...
  const sendRequest = useCallback(
    <T>(message: { requestId: string } & Record<string, unknown>): Promise<T> => {
  ...
  const handlePendingResponse = useCallback(
    (requestId: string, success: boolean, data: unknown, error?: string) => {
  ...
  const sendSettings = useCallback(
    (agent: AgentType, settings: AgentSettings) => {
      send({
        channel: "agent",
        type: "settings",
        agent,
  ...
  const writeFile = useCallback(
    async (path: string, content: string): Promise<FileInfo> => {
      const requestId = crypto.randomUUID();
  ...
  const listFiles = useCallback(
    async (path: string): Promise<DirListing> => {
      const requestId = crypto.randomUUID();
  ...
  const mkdir = useCallback(
    async (path: string): Promise<{ path: string }> => {
      const requestId = crypto.randomUUID();
  ...
  const renameFile = useCallback(
    async (oldPath: string, newPath: string): Promise<{ path: string }> => {
      const requestId = crypto.randomUUID();
